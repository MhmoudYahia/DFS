...
var (
	ip           = flag.String("ip", "", "The public/LAN IP address of this Data Keeper") // CHANGED: new flag for Data Keeper's IP
...
func main() {
	...
	// CHANGED: Use provided IP (or fallback to a default) to advertise the real address.
	addr := *ip
	if addr == "" {
		addr = "localhost"
	}
	s := &server{
		address:      fmt.Sprintf("%s:%s", addr, *port), // CHANGED: use provided IP instead of "localhost"
		dataPort:     *dataPort,
		downloadPort: *downloadPort,
	...
	log.Printf("Data Keeper server started on %s with upload port %s and download port %s...", s.address, *dataPort, *downloadPort)
